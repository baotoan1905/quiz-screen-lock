...
- GitHub Actions CI/CD workflows:
  - `dev-build.yml`: builds `QuizLock.exe` on push to `dev`, uploads as artifact `QuizLock-dev-{run_number}` (retained 7 days)
  - `prod-release.yml`: builds `QuizLock.exe` on push to `main`, auto-increments semver tag, creates a GitHub Release with the exe attached
